...
TEN 的核心运作机制是图（graph），一个图可以包含多个节点（nodes）和多个连接（connections）。以下是一个仅涉及单个 TEN app 的图定义示例：
...
然而，有时我们希望将一个图拆分成多个图，并希望这些图可以被复用。这时就引入了子图（subgraph）的概念。子图被设计为一种语法糖（syntax sugar），子图最终会先被展平在所属的大图中，然后才使用相同的机制来启动大图。这意味着，子图的引入并不会改变图原本的运作机制。子图机制的几个设计原则如下：

1. 子图虽然可以嵌入到其他图中，但本身就是一个正常的图，如有需要可以单独启动。
2. 任何图都可以提供额外的信息，使工具能更容易理解图的结构。工具可以显示并使用这些信息来提升开发体验，更容易对图进行操作。但这些信息仅供工具使用，不会导致运行时的图结构变得复杂。
3. 子图最终会被工具或 TEN runtime 展平成与现有图结构相同的图，继而使用相同的运行机制来启动整个图。
4. 引入子图不会使图的 JSON 结构变得复杂，而是保持图的 JSON 简单形态，通过工具来提升开发者体验和开发效率。这意味着，引入子图后，理解图的 JSON 本身可能需要一些人工转换，但设计不会为了简化这种人工转换而做额外设计，而是依赖工具来消除这种转换需求，从而在不增加图复杂度的同时提升开发体验。

以下是一个 `subgraph.json` 的示例，它同时也是一个正常的图，可以单独使用：
...
虽然在图的定义中使用了 `"extension": "graph_any_name:ext_d"` 这种描述，看似让开发者接触到了子图内部的细节，但借助图的 `expose_msgs` 信息以及开发工具的辅助，开发者在使用工具时可以不需接触子图内部的细节。当工具按照开发者的意图操作时，工具最终会自动为开发者处理好这些细节，并生成正确的图定义。这符合子图的初衷，子图更多是一种语法糖，不增加图定义本身的复杂度，而是提供信息让工具完成足够的封装，提升开发体验。

例如，当开发者在工具中看到一个图时，工具可以呈现该图的 expose_msgs 信息，如该图暴露了一个名为 B 的 cmd。当开发者通过工具将某个 extension 的 cmd B 连接到图所暴露的 B cmd 时，工具会通过 expose_msgs 的信息了解到这个命令 B 要连接的目标实际上是图内的某个 extension，并将这个信息写入最终的图定义中，即上面 JSON 中呈现的样子。

总结来说，子图机制没有在引用处提供修补子图定义的机制，因为这相当于触及了子图的内容，违反了子图的初衷，而且也会使子图的使用变得复杂，并不一定是好事。因此，开发者如果想要调整子图的内容，应该直接修改子图的定义本身，而不是在引用处进行修补。
