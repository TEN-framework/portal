...
在 TEN 框架中，我们鼓励开发者创建自己的扩展（Extension）并将其发布到 TEN 扩展市场，供其他开发者使用。为了便于开发者选择和替换扩展，我们需要定义标准化的接口规范，让开发者能够根据这些标准接口轻松找到符合需求的扩展。
...
当要把实现接口 A 的 extension 替换成接口 B，需要接口 A 兼容于接口 B，才能进行替换。接口之间的兼容规则有两种模式，如下：

**严格模式：**

- 接口 A 的所有 msg 都必须出现在接口 B 中，且接口 A 中的 msg 的 schema 必须兼容于接口 B 中的 msg 的 schema。

**宽松模式：**

- 只有有连线（也就是有 connection）的接口 A 中的 msg 必须出现在接口 B 中，且接口 A 中的这些 msg 的 schema 必须兼容于接口 B 中的同名 msg 的 schema。

严格模式跟宽松模式是用户可选的，适应于不同的情况，例如如果确定 extension 所实现的接口 A 中的某个 msg X 一定不会被使用到，那因为 msg X 的不兼容或不存在而导致无法替换成想要的另一个 extension 可能会太严格。
...
**概述**：这种方式是将标准接口定义文件打包成 TEN 系统包，然后在 `manifest.json` 中引用。并且同时也可以在 manifest.json 的 dependencies 字段内依赖这个系统包，使得可以在安装这个扩展的时候就一并安装所依赖的标准接口系统包，达到最佳的开发体验。借由这个方法，可以利用 TEN 包的版本机制来实现标准接口的版本管理，并且可以利用 TEN 包的依赖机制来实现标准接口的自动安装。
...
需要注意的是，TEN 系统包的版本号并不只代表标准接口的版本号，而是代表整个系统包的版本号。因此，如果系统包内的资源有破坏性变更，则应该要升级系统包的主版本号。例如，系统包内的 JSON 文件的摆放位置或者文件名称的不同，纵使没有改变 JSON 文件的内容，也算是该系统包的破坏性变更，因为对使用者来说，使用的方式需要被改变，所以应该要升级该系统包的主版本号。

因此不能仅仅只用系统包的版本号来判断标准接口的兼容性。标准接口的兼容性还是需要通过接口的兼容规则来判断。
...
## 如何从云商店中筛选想要的 Extension

标准接口与云商店的关系在于将标准接口包装成 TEN 系统包。有以下两种方式可以搜寻出符合某些条件的 extensions：

1. **依赖搜索**：搜寻依赖某个系统包的 extensions。由于云商店的 extensions 的 metadata 中，具有依赖哪些包的资讯，所以云商店可以据此筛选出依赖某个系统包的所有包。

2. **标签搜索**：透过 TEN package 的 tag 来快速的搜寻出符合某些条件的 extensions，选择安装后，再进行替换的动作。如果是 TEN 官方认证过的 extensions，甚至可以打上 `ten:` 开头的 tag 来表明这是一个官方认证过的 extensions，确实符合所宣告的标准接口的定义。
