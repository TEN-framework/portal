...
3. For the name field give it a unique name `hello_world` in the graph.
4. Click "Add Node" to place the extension node on the canvas.
5. You should see the `hello_world` node appear on the canvas.
