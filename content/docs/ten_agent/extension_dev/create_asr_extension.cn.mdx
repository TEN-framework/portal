...
async def start_connection(self) -> None:
    ...
    try:
        ...
        self.ten_env.log_error(f"failed to connect to deepgram: {e}", category="vendor")
        raise
...
async def _on_open(self, *args, **kwargs) -> None:
    ...
    self.ten_env.log_info("Deepgram connection opened", category="vendor")

async def _on_close(self, *args, **kwargs) -> None:
    """连接关闭"""
    self.is_connected_flag = False
    self.ten_env.log_warn("Deepgram connection closed", category="vendor")
...
async def _on_transcript(self, *args, **kwargs) -> None:
    ...
    try:
        ...
        self.ten_env.log_error(f"Error processing transcript: {e}", category="vendor")

async def _on_error(self, *args, **kwargs) -> None:
    """处理错误"""
    error = args[1] if len(args) > 1 else "Unknown error"
    self.ten_env.log_error(f"Deepgram error: {error}", category="vendor")
```
...
class MyAsrExtension(AsyncASRBaseExtension):
    ...
    async def start_connection(self) -> None:
        ...
        try:
            ...
            # 记录供应商错误日志
            self.ten_env.log_error(
                f"vendor_error: start_connection failed: {e}",
                category=LOG_CATEGORY_VENDOR,
            )
