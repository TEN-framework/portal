...
👉 You can find it here: [**voice-assistant-sip-twilio example on GitHub**](https://github.com/TEN-framework/ten-framework/tree/main/ai_agents/agents/examples/voice-assistant-sip-twilio)
...
8. Start talking — your **AI phone system** will handle the entire conversation!
...
1. **Phone → Twilio**: Person speaks into phone
2. **Twilio → WebSocket**: Audio sent as base64-encoded chunks
3. **WebSocket → TEN**: Audio forwarded to ASR extension
4. **ASR → LLM**: Speech converted to text, sent to AI
5. **LLM → TTS**: AI response converted to speech
6. **TTS → WebSocket**: Audio sent back to Twilio
7. **WebSocket → Twilio → Phone**: AI voice plays through phone
