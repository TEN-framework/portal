...
export function GuideSteps({ steps, locale }: { steps: Step[]; locale: "en" | "cn" }) {
    ...
    <div className="flex flex-col gap-6">
      <div className="rounded-3xl border p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="guide-step-badge inline-flex size-10 items-center justify-center rounded-full">{doneCount}</span>
            <div>
              <p className="font-semibold text-sm">{locale === "cn" ? "已完成步骤" : "Steps completed"}</p>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              onClick={() => setCompleted(Object.fromEntries(steps.map(s => [s.number, true])))}
            >
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {steps.map((step) => {
          ...
          return (
            <div key={step.number} className="guide-panel rounded-3xl p-6 transition-transform hover:-translate-y-0.5">
              <div className="guide-text-muted flex items-center gap-3 font-semibold text-sm">
                <button
                  aria-pressed={isDone}
                  onClick={() => setCompleted(prev => ({ ...prev, [step.number]: !isDone }))}
              ...
              <ul className="mt-4 space-y-2 text-sm">
                {step.bullets.map((entry) => (
                  <li key={`${step.number}-${entry}`} className="guide-text-muted flex gap-2">
