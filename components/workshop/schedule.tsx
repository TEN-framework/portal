...
import { getTimeZoneAbbreviation, formatSessionTimeRange } from '@/lib/date'
...
export function WorkshopSchedule({
  ...
  const timezoneLabel = getTimeZoneAbbreviation(new Date(sessions[0].start), timezone)
    ...
    <section className="border-t border-slate-200 bg-slate-50 py-16 dark:border-slate-800 dark:bg-slate-950">
      <div className="mx-auto max-w-5xl px-6">
        <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
          ...
          {sessions.map((session, index) => {
            ...
            return (
              <li key={session.id} className="relative ml-4">
                <span className="absolute -left-8 top-2 flex size-4 items-center justify-center rounded-full border border-emerald-400 bg-white dark:border-emerald-500 dark:bg-slate-950" />
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:border-emerald-200 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:hover:border-emerald-500/50">
                  <div className="mb-4 flex flex-wrap items-center gap-3 text-sm text-slate-500 dark:text-slate-300">
                    ...
                    <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1 text-xs font-medium uppercase tracking-wide text-slate-600 dark:bg-slate-800 dark:text-slate-200">
                    ...
                    {session.tags?.map((tag) => (
                      <span
                        key={tag}
                        className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-medium uppercase tracking-wide text-slate-600 dark:bg-slate-800 dark:text-slate-200"
                      >
                        {tag}
                      </span>
                    ))}
